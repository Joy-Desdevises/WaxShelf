    ...
    >
      <div className="safe-bottom relative flex w-full max-h-[70vh] flex-col rounded-t-2xl bg-[#111] shadow-2xl sm:max-h-[80vh] sm:max-w-sm sm:rounded-xl">
        ...
        <div className="flex items-center justify-between border-b border-[#222] p-4">
          <h2 className="font-semibold text-white">{t('likedVinyls.title')}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {isLoading ? (
            <p className="p-4 text-center text-sm text-[#999]">{t('likedVinyls.loading')}</p>
          ) : list.length === 0 ? (
            <p className="p-4 text-center text-sm text-[#999]">{t('likedVinyls.empty')}</p>
          ) : (
